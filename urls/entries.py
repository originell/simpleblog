...
urlpatterns = patterns('django.views.generic.date_based',
    (r'^$', 'archive_index', entry_info_dict, 'archive_index'),
    (r'^(?P<year>\d{4}/$', 'archive_year', entry_info_dict, 'archive_year'),
    (r'^(?P<year>\d{4}/(?P<month>\w{3}/$', 'archive_month',
        entry_info_dict, 'archive_month'),
    (r'^(?P<year>\d{4}/(?P<month>\w{3}/(?P<day>\w{2}/$', 'archive_day',
        entry_info_dict, 'archive_day'),
    (r'^(?P<year>\d{4}/(?P<month>\w{3}/(?P<day>\w{2}/(?P<slug>[-\w]+)/$',
        'entry_detail', entry_info_dict, 'entry_detail'),
)
